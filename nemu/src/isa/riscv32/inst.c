...
static inline int64_t sext32(word_t value) {
  return (value & UINT32_C(0x80000000)) ?
      (int64_t)value - (INT64_C(1) << 32) : (int64_t)value;
}

static inline word_t sra32(word_t src, uint32_t shamt) {
  if (shamt == 0) {
    return src;
  }
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = sext32(src1) < sext32(imm));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = sra32(src1, BITS(imm, 4, 0)));
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if (sext32(src1) <  sext32(src2)) s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if (sext32(src1) >= sext32(src2)) s->dnpc = s->pc + imm);
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J,
      R(rd) = s->pc + 4;
      s->dnpc = s->pc + imm;
      IFDEF(CONFIG_FTRACE, ftrace(s->pc, s->dnpc, s->isa.inst, rd,
          BITS(s->isa.inst, 19, 15), imm));
  );
  ...
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(rd) = sext32(src1) < sext32(src2));
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = src1 < src2);
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(rd) = src1 ^ src2);
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(rd) = src1 >> BITS(src2, 4, 0));
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = sra32(src1, BITS(src2, 4, 0)));
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(rd) = src1 & src2);

  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = (word_t)((uint64_t)src1 * src2));
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (word_t)((uint64_t)(sext32(src1) * sext32(src2)) >> 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = (word_t)((uint64_t)(sext32(src1) * (int64_t)src2) >> 32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (word_t)(((uint64_t)src1 * src2) >> 32));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = src2 == 0 ? UINT32_MAX : (word_t)(sext32(src1) / sext32(src2)));
